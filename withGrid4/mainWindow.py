...
class AI():
    ...
    def play(self,window,car):
        if self.playEpisodeCounter < self.maxPlayEpisode:

            if self.playStepCounter < self.maxPlayStep and self.stopFlag ==False:
...
class MyWindow(pyglet.window.Window):
    ...
    def on_draw(self):
        ...
        self.centerLabel = pyglet.text.Label(("Episode = " + str(self.episodeCounter)+ " , Step = " + str(self.stepCounter)+ " , Complete = " + str(self.completeC)),
    ...
    def on_key_press(self, symbol,modifiers):
        if symbol == key.UP:
            ...
            tempA = np.loadtxt('auto-save1.txt', dtype =float)
